...
//        System.out.println(System.currentTimeMillis()-start);
    ...
    public static String decompose(String data) {
        StringBuilder sb = new StringBuilder();

        for(int i = 0; i < data.length(); i++) {
            char nTmp = data.charAt(i);

            sb.append(decompose(nTmp));
        }

        return sb.toString();
    }

    public static String decompose(char nTmp) {
        if(nTmp>='가' && nTmp<='힣') {
            nTmp -= 0xAC00;
            ...
            if(jongDecompose.containsKey(choResult)) {
            ...
            if (jong != 0) {
                ...
                if(jongDecompose.containsKey(jongResult)) {
