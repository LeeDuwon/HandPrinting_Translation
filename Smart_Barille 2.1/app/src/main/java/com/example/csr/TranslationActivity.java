...
public abstract class TranslationActivity extends AppCompatActivity {
    ...
    protected void saveWord(final String word) {
        ...
        // 단어 중복 체크
        Query query = reference.whereEqualTo("word", word).limit(1);
                ...
                if (task.isSuccessful()) {
                    if (task.getResult() != null) {
                        if (task.getResult().size() == 0) {
                            // 중복 아님
                            // 등록
                            Word item = new Word(word, result, (startKo ? Constants.WordLanguageKind.ENGLISH : Constants.WordLanguageKind.KOREAN),
